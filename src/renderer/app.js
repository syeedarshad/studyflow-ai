...
function handleDelegatedEvent(event) {
  ...
  if (action === 'sendCoachMessage' && args[0] === undefined) {
    handler();
  } else {
    handler(...args);
  }
  ...
  try {
    ...
    slot.innerHTML = `
      <div class="coach-banner">
        ...
        <div class="coach-banner-body">
          You planned <strong>"${task.title}"</strong> (${task.category}) but it's now overdue.
    ...
    <div style="background:${isDone ? 'rgba(74,222,128,0.06)' : 'var(--surface-2)'};border:1px solid ${isDone ? 'var(--success)' : 'var(--border)'};border-radius:var(--radius-sm);padding:10px 14px;animation:planItemIn 0.4s ease backwards;animation-delay:${index*0.06}s">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <span style="font-size:13px;font-weight:600;color:var(--text)">${quest.title}</span>
        </div>
        <span style="font-size:11px;color:var(--accent);font-weight:700">+${quest.xp_reward} XP</span>
      </div>
      <div style="font-size:11px;color:var(--text-3);margin-bottom:6px">${quest.description}</div>
...
function showTaskPlanApproval(plan, originalPrompt) {
  ...
  const itemsHtml = tasks.map((t, i) => `
    <div class="plan-preview-item" data-idx="${i}">
      ...
      <span class="pp-title">${t.title}</span>
...
async function showEditTaskModal(id) {
  ...
  showModal('✏️ Edit Task', `
    <div class="form-group">
      <label class="form-label">Title</label>
      <input class="form-input" id="edit-task-title" value="${task.title}">
    </div>
    <div class="grid-2" style="gap:10px">
      <div class="form-group">
        <label class="form-label">Category</label>
    ...
    <div class="form-group">
      ...
      <textarea class="form-textarea" id="edit-task-notes">${task.notes||''}</textarea>
    </div>
        ...
        <div class="card ai-shimmer">
          ...
          ${topTask ? `
            <div style="background:var(--surface-2);border-radius:var(--radius-sm);padding:10px 12px;margin-bottom:12px">
              ...
              <div style="font-size:13px;font-weight:600;color:var(--text)">${topTask.title}</div>
    ...
    <div class="card ${goal.status==='completed'?'':'ai-shimmer'}" style="position:relative">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px">
        <div style="min-width:0">
          <div style="font-size:15px;font-weight:700;color:var(--text);margin-bottom:4px">${goal.title}</div>
          ${goal.description ? `<div style="font-size:12px;color:var(--text-3)">${goal.description}</div>` : ''}
        </div>
    ...
    <div class="plan-preview-list" style="max-height:400px">
      ...
      ${tasks.map(t => `
        <div class="plan-preview-item" style="${t.status==='completed'?'opacity:0.5':''}">
          ...
          <span class="task-category cat-${t.category.toLowerCase().replace(/\s+/g,'_')}" style="font-size:10px">${t.category}</span>
          <span class="pp-title" style="${t.status==='completed'?'text-decoration:line-through':''}">${t.title}</span>
  ...
  return `
    <div class="card" style="margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px">
        <div>
          <div style="font-size:15px;font-weight:700;color:var(--text)">${roadmap.title}</div>
          <div style="font-size:12px;color:var(--text-3)">Target: ${roadmap.target_role} · ${roadmap.total_months} months</div>
        </div>
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${(roadmap.milestones||[]).map(m => `
          <div style="background:var(--surface-2);border-radius:var(--radius-sm);padding:12px 14px;border-left:3px solid ${m.status==='completed'?'var(--success)':'var(--border)'}">
            ...
            <div style="font-size:12px;color:var(--text-2);margin-bottom:8px">${m.description||''}</div>
    ...
    <div style="max-height:400px;overflow-y:auto;display:flex;flex-direction:column;gap:8px;margin-bottom:14px">
      ${milestones.map(m => `
        <div style="background:var(--surface-2);border-radius:var(--radius-sm);padding:12px 14px">
          ...
          <div style="font-size:12px;color:var(--text-2);margin-bottom:6px">${m.description}</div>
      ...
      <div class="plan-preview-list" style="margin-bottom:12px">
        ${examPlan.daily_plan.map(a => `
          <div class="plan-preview-item">
            ...
            <span class="pp-title">${a.activity}</span>
        ...
        ${blocks.length === 0
          ...
          : blocks.map(b => `
            <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">
              ...
              <span style="font-size:13px;color:var(--text);flex:1">${b.title}</span>
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px">
            ${s.subjects.map(sub => `
              <div class="insight-tag ${sub.priority==='high'?'warn':''}">
                📚 ${sub.subject_name}${sub.exam_date?` · ${formatDate(sub.exam_date)}`:''}
              </div>
            `).join('')}
          </div>
        ` : ''}
...
async function renderCoachChat(container) {
  const res      = await window.studyflow.coachChatGetHistory();
  ...
  container.innerHTML = `
    ...
    <div class="card" style="display:flex;flex-direction:column;height:calc(100vh - 220px)">
      <div id="chat-messages" style="flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:12px;padding:4px 0;margin-bottom:16px">
        ${messages.length===0 ? `
          <div style="text-align:center;color:var(--text-3);font-size:13px;padding:32px">
      ...
      <div style="display:flex;gap:10px;border-top:1px solid var(--border);padding-top:14px">
        <input class="form-input" id="chat-input" placeholder="Ask your coach anything..."
          style="flex:1" data-action="sendCoachMessage" data-event="keydown">
        <button class="btn btn-primary" id="chat-btn" data-action="sendCoachMessage">Send</button>
      </div>
    </div>
  `;
      ...
      ${notes.map(n => `
        <div class="card ${n.is_pinned?'ai-shimmer':''}" style="cursor:pointer" data-action="showEditNoteModal" data-note-id="${n.id}">
          <div style="display:flex;justify-content:space-between;align-items:flex-start">
            <div class="card-title">${n.is_pinned?'📌 ':''}${n.title||'Untitled'}</div>
          ...
          <div style="font-size:13px;color:var(--text-2);line-height:1.5;max-height:80px;overflow:hidden">${(n.content||'').slice(0,200)}</div>
...
async function showEditNoteModal(id) {
  ...
  showModal('✏️ Edit Note', `
    <div class="form-group"><label class="form-label">Title</label><input class="form-input" id="edit-note-title" value="${note.title||''}"></div>
    <div class="form-group"><label class="form-label">Content</label><textarea class="form-textarea" id="edit-note-content" rows="6">${note.content||''}</textarea></div>
...
async function renderAchievements(container) {
  ...
  container.innerHTML = `
    ...
    <div class="progress-bar" style="margin-bottom:24px"><div class="progress-fill" style="width:${Math.round((earnedIds.size/allBadges.length)*100)}%"></div></div>
    <div class="grid-3" style="gap:14px">
      ${allBadges.map(b => {
        const isEarned = earnedIds.has(b.id);
        const earnedAt = earned.find(a=>a.badge_id===b.id)?.earned_at;
        return `
          <div class="card ${isEarned?'ai-shimmer':''}" style="text-align:center;padding:20px;${isEarned?'':'opacity:0.5'}">
            <div style="font-size:36px;margin-bottom:8px">${b.icon}</div>
            <div style="font-size:13px;font-weight:700;color:${isEarned?'var(--accent)':'var(--text)'};margin-bottom:4px">${b.name}</div>
            <div style="font-size:11px;color:var(--text-3)">${b.desc}</div>
            ${isEarned && earnedAt ? `<div style="font-size:10px;color:var(--success);margin-top:6px">Earned ${new Date(earnedAt).toLocaleDateString()}</div>` : ''}
          </div>
        `;
      }).join('')}
    </div>
...
async function renderSettings(container) {
  const res      = await window.studyflow.db('getAllSettings');
  ...
  container.innerHTML = `
    ...
    <div class="card" style="max-width:600px;margin-bottom:16px">
      ...
      <div class="form-group">
        ...
        <input class="form-input" id="setting-name" value="${settings.user_name||'Student'}">
      </div>
      <div class="form-group">
