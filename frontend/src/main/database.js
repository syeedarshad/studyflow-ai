...
class StudyFlowDB {
  ...
  init() {
    ...
    // Enforce foreign keys from this point on. Must come AFTER the schema
    // migrations above — the FK-action migration itself needs to briefly
    // turn this off while it rebuilds tables (SQLite requirement).
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // FOREIGN KEY ACTIONS (Phase 1 hardening)
  // ═══════════════════════════════════════════════════════════════════════
  // SQLite can't ALTER a FOREIGN KEY clause onto an existing table — the
  // only way to add ON DELETE behavior to already-created tables is the
  // standard SQLite "12-step" rebuild: create a new table with the right
  // schema, copy the data across, drop the old table, rename the new one.
  //
  // This runs ONCE (guarded by a settings flag), inside a single
  // transaction (all six tables succeed or none of them do), with a
  // row-count check after every copy and a full database file backup
  ...
  migrateForeignKeyActions() {
    ...
    // ── Safety net: back up the raw db file before touching schema ──────
    try {
      const fs = require('fs');
      if (this.dbPath && fs.existsSync(this.dbPath)) {
        const backupPath = `${this.dbPath}.pre-fk-migration-${Date.now()}.bak`;
        fs.copyFileSync(this.dbPath, backupPath);
        logger.info(`Pre-migration backup written to ${backupPath}`);
      }
    } catch (err) {
      logger.warn('Could not write pre-migration backup (continuing anyway):', err.message);
    }
    ...
    // CRITICAL: PRAGMA foreign_keys can only be changed OUTSIDE an active
    // transaction — SQLite silently no-ops the pragma if called inside a
    // BEGIN...COMMIT. Setting this inside the transaction() below looked
    // correct but was a no-op, which meant FK enforcement stayed ON
    // through every DROP TABLE in the rebuild sequence — and because the
    // rebuilt sessions/time_blocks tables already had their new
    // "ON DELETE SET NULL" clause active by the time `tasks` was dropped
    // and recreated, SQLite correctly-per-the-still-enabled-constraint
    // nulled sessions.task_id / time_blocks.task_id right then, silently
    // destroying real task links. Verified via a reproduction against a
    // real SQLite engine before and after this fix; see verify-database.js
    // in the test suite for the regression test that guards this.
    this.db.pragma('foreign_keys = OFF');

    const migration = this.db.transaction(() => {
      // (Do NOT re-set the pragma here — see comment above.)

      // sessions.task_id -> tasks(id): a task is only ever soft-deleted
      // today, but if that ever changes, keep the session history and
      // just unlink it rather than destroying analytics data.
      rebuild('sessions', `
      ...
      // (deleteCareerRoadmap already hard-deletes milestones first, so
      // this CASCADE is defense-in-depth, not a behavior change.)
      rebuild('roadmap_milestones', `
      ...
      `, 'id, roadmap_id, month_number, title, description, skills, projects, status, created_at');

      // semester_subjects.semester_id -> semesters(id) NOT NULL: same
      // reasoning — a subject row makes no sense without its semester.
      // (deleteSemester already hard-deletes subjects first; CASCADE here
      // is defense-in-depth for any future direct-delete code path.)
      rebuild('semester_subjects', `
      ...
      `, 'id, semester_id, subject_name, exam_date, credits, priority, created_at');

      // tasks.goal_id -> goals(id): goals are only ever soft-deleted, but
      // keep this defensive — unlink rather than destroy the task.
      rebuild('tasks', `
        CREATE TABLE tasks (
          id                  INTEGER PRIMARY KEY AUTOINCREMENT,
          title               TEXT NOT NULL,
      ...
      // daily_quests.goal_id -> goals(id): same reasoning as tasks.
      ...
      // inside the transaction regardless of the enforcement toggle.)
      const violations = this.db.pragma('foreign_key_check');
      if (violations && violations.length > 0) {
        throw new Error(`FK migration left ${violations.length} orphaned row(s) — aborting: ${JSON.stringify(violations.slice(0, 5))}`);
      }
    });
    ...
    try {
      ...
      // The transaction wrapper already rolled everything back; the
      // database is exactly as it was before this method ran. Do NOT
      // set the completion flag, so this is retried next launch.
      logger.startupError('migrateForeignKeyActions', err);
    }
  }

  /**
   * createPerformanceIndexes — the dedup UNIQUE indexes already exist
   * (idx_task_unique, idx_planner_entries_date, idx_active_goal_title).
   * These are read-pattern indexes: every one matches a WHERE/ORDER BY
   * that already appears in this file for date-range or status lookups.
   * Cheap at current data volumes, protects against slowdown as history
   * accumulates over semesters/years of use.
   */
  ...
  /**
   * migrateApiKeysToEncrypted — one-time migration for installs that
   * already have plaintext Gemini/Groq keys saved from before encryption
   * was added. Runs once, guarded by a settings flag, so existing users'
  ...
  getAIContextSummary() {
    ...
    return {
      ...
      studentProfile: {
        ...
        collegeName:       memory.college_name        || null,
        careerGoal:        memory.career_goal         || null,
        dreamCompanies:    memory.dream_companies      || null,
        knownLanguages:    memory.known_languages      || null,
        targetLanguage:    memory.target_language      || null,
        weakSubjects:      memory.weak_subjects        || null,
        strongSubjects:    memory.strong_subjects      || null,
        collegeStartTime:  memory.college_start_time   || null,
        collegeEndTime:    memory.college_end_time     || null,
        dailyStudyHours:   memory.daily_study_hours    || null,
        wakeTime:          memory.wake_time            || null,
        sleepTime:         memory.sleep_time           || null,
        exercise:          memory.exercise             || null,
        distractions:      memory.distractions         || null,
        bestStudyWindows:  memory.best_study_windows        || null,
        weeklyWorkload:    memory.weekly_workload_summary   || null
      }
    };
  }
